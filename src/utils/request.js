...
import { messag, Model, notification, Modal } from 'ant-design-vue'
...
//拦截请求
CHEN_REQUEST.interceptors.request.use((config) => {
...
//拦截响应
CHEN_REQUEST.interceptors.response.use((config) => {
    return config
}, (error) => {
    if (error.response) {
...
const request = {
    post(url, params) {
        return CHEN_REQUEST.post(url, params, {
    ...
    put(url, params) {
        return CHEN_REQUEST.put(url, params, {
            transformRequest: [(params) => {
                let result = ''
                Object.keys(params).forEach((key) => {
                    if (!Object.is(params[key], undefined) && !Object.is(params[key], null)) {
    ...
    get(url, params) {
        ...
        return CHEN_REQUEST.get(`${url}${_params}`)
    },
    delete(url, params) {
        let _params
        if (Object.is(params, undefined)) {
        ...
        return CHEN_REQUEST.delete(`${url}${_params}`)
    },
    export (url, params = {}) {
